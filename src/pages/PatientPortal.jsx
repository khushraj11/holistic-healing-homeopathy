      ...
      <div style={{ padding: '1.5rem', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ color: '#888', fontSize: '0.88rem' }}>📞 {patient.phone}</p>
          </div>
          <button onClick={() => loadRecords(patient.phone)} style={{
            background: 'rgba(74,122,80,0.1)', border: '1px solid rgba(74,122,80,0.3)',
            color: '#4a7a50', padding: '0.5rem 1rem', borderRadius: '20px',
            cursor: 'pointer', fontFamily: 'Jost', fontSize: '0.82rem'
          }}>🔄 Refresh</button>
        </div>
            ...
            <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', marginTop: '1rem', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
              ...
              <form onSubmit={bookAppointment}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
                  <div>
                    <label style={{ fontSize: '0.8rem', color: '#4a7a50', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '0.4rem' }}>Preferred Date</label>
                    <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} required style={inputStyle} />
                  </div>
                  <div>
                    <label style={{ fontSize: '0.8rem', color: '#4a7a50', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '0.4rem' }}>Preferred Time</label>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <textarea value={form.symptoms} onChange={e => setForm({...form, symptoms: e.target.value})}
                    placeholder="Describe your symptoms..." required rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
                </div>
                <button type="submit" disabled={booking} style={{
                  width: '100%', padding: '0.9rem', background: '#4a7a50',
                  color: 'white', border: 'none', borderRadius: '12px',
                  fontFamily: 'Jost', fontSize: '0.95rem', cursor: 'pointer'
                }}>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem', color: '#aaa', background: '#f9f9f9', borderRadius: '12px' }}>
              ...
              <p style={{ marginBottom: '0.5rem' }}>No appointments found.</p>
              <p style={{ fontSize: '0.82rem' }}>Book an appointment above to get started!</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {records.map((rec, i) => (
                <div key={i} style={{
                  ...
                  background: rec.diagnosis ? '#f9fdf9' : '#fafafa'
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                    <span style={{ background: 'rgba(122,158,126,0.15)', color: '#4a7a50', padding: '0.3rem 1rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 500 }}>
                      Visit #{i + 1}
                    </span>
